...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const formattedQuote = {
      ...
      dayHigh: Number(
        (meta.regularMarketDayHigh || Math.max(...highs.filter((h) => h !== null)) || currentPrice).toFixed(2),
      ),
      dayLow: Number(
        (meta.regularMarketDayLow || Math.min(...lows.filter((l) => l !== null)) || currentPrice).toFixed(2),
      ),
